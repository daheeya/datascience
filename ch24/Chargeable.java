...
public class Chargeable {
    ...
    protected int getFee(int price){
        return (int)(price*this.cardType.getFee());
    }
    protected int getTax(int price){
        return (int)(price*this.cardType.getTax());
    }
    protected void payment(int price){
       datas.add(new TransactionData(new Date(),price,getTax(price),getFee(price)));
